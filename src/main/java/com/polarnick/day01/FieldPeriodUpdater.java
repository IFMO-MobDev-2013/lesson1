...
public class FieldPeriodUpdater implements Runnable {
    ...
    public FieldPeriodUpdater(int threadNum, int fromY, int toY, boolean cornerCases, PeriodHolder periodHolder, int width, int height, int[] field, int[] tmp, int[] fieldNext, int[] tmpNext) {
    ...
    public void setNewFieldWriteAt(int[] field) {
        this.tmp = field;
    }
    ...
    private void updateField5(int fromY, int toY, boolean cornerCases) {
        profiler.in(FieldUpdaterThreadFunctions.updateField5);
        {
            ...
            toY = Math.min(toY, cornerY[1]);
            int index = width * fromY + 1;
            boolean everythingChanges = true;
            for (int y = fromY; y < toY; ++y) {
                for (int x = 1; x < width - 1; ++x) {
            ...
            if (!everythingChanges) {
                periodHolder.setStillPeriodStarted(false);
            }
        }
        if (cornerCases) {
            calculateCornerCases();
